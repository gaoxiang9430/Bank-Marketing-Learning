...
rdesc <- makeResampleDesc("CV", iters=10L)  # CV cross-validation
...
allTraintask <- mlr::filterFeatures(allTraintask, method = "cforest.importance", abs = 6)
model <- mlr::train(rf.lrn, allTraintask)
#print(model)
#attributes(model)
...
write.csv(solution, file = '~/Documents/R/data/cforest_1211.csv', row.names = F)
